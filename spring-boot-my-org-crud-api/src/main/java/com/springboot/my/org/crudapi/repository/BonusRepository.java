...
public class BonusRepository implements BonusDAO{
	...
	public List<Bonus> getWorkerWithBonus(int id) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("SELECT * FROM bonus WHERE WORKER_REF_ID = ?");
        ps.setInt(1, id);
	...
	public List<Bonus> getWorkerBonusByDept(String department) throws SQLException {
		List<Bonus> bonusList = new ArrayList<>();
        PreparedStatement ps = conn.prepareStatement("SELECT BONUS.* FROM bonus INNER JOIN worker ON worker.WORKER_ID = bonus.WORKER_REF_ID "
